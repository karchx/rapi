...
RSpec::describe "Posts with authentication", type: :request do
  ...
  let!(:auth_header) { { 'Authorization' => "Bearer #{user.auth_token}" } }
    ...
    context "with valid auth" do
      context "when requisting other's author post" do
        ...
        context "when post is draft" do
          before { get "/posts/#{other_user_post_draft.id}", headers: auth_header }
  ...
  describe "POST /posts" do
    context "with valid auth" do
      before { post "/posts", params: create_params, headers: auth_header }
  ...
  describe "PUT /posts" do
    context "with valid auth" do
      context "when updating users's post" do
        before { put "/posts/#{user_post.id}", params: update_params, headers: auth_header }
        context "payload" do
          subject { payload }
          it { is_expected.to include(:id, :title, :content, :published, :author) }
      ...
      context "when updating other user's post" do
        before { put "/posts/#{other_user_post.id}", params: update_params, headers: auth_header }
        context "payload" do
          subject { payload }
          it { is_expected.to include(:error) }
        end
